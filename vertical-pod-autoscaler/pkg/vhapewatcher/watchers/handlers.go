...
func (w *Watchers) onDeploymentUpdate(_, _ interface{}) {
	klog.V(5).InfoS("Ignoring Deployment update event")
}
...
func (w *Watchers) onDeploymentDelete(_ interface{}) {
	klog.V(5).InfoS("Ignoring Deployment delete event")
}
...
func (w *Watchers) onWatchedNamespaceUpdate(_, _ interface{}) {
	klog.V(5).InfoS("Ignoring VhapeWatchedNamespace update event")
}
...
func (w *Watchers) enqueueDeploymentFromVPA(vpa *vpav1.VerticalPodAutoscaler, reason string) {
	...
	if !isDeploymentTarget(ref.APIVersion, ref.Kind, ref.Name) {
		klog.V(5).InfoS("Ignoring VPA event for non-Deployment target", "vpa", klog.KObj(vpa), "apiVersion", ref.APIVersion, "kind", ref.Kind, "name", ref.Name, "reason", reason)
		return
	}
